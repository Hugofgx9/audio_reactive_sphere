...
void main() {
	...
	float density = u_density + noise3(phi, theta, 100. + u_time * 0.3) * u_noise_amount;
	...
	vec3 noise_pos = sin(pos.xyz * 2.);

	pos.z += noise3(noise_pos.x, noise_pos.y, u_time * 0.6 + pos.z) * u_noise2_amount * 0.7;
	pos.x += noise3(noise_pos.y, noise_pos.x + map(sin(u_time),-1.,1.,0.,1.), u_time * 0.3) * u_noise2_amount * 0.7;
	pos.y += noise3(noise_pos.y, noise_pos.z, u_time * 0.4) * u_noise2_amount * 0.7;
